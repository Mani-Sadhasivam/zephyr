...
void main(void)
{
	...
	gpio_dev = device_get_binding(CONFIG_GPIO_5_NAME);
	if (gpio_dev == NULL) {
		printk("Device not found\n");
		return;
	}

	gpio_pin_configure(gpio_dev, GPIO, GPIO_DIR_OUT);
	gpio_pin_write(gpio_dev, GPIO, 1);

	dev = device_get_binding(CONFIG_I2S_3_NAME);
	if (dev == NULL) {
		printk("Device not found\n");
		return;
	}
	...
	ret = i2s_write(dev, test_pcm, 48000);
	if (ret < 0) {
		printk("Write failed\n");
		return;
	}
